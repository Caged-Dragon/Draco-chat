...
-- 1b) NUMERIC ACCOUNT ID (v5.2) ------------------------------------
-- Every account also gets a permanent, database-assigned numeric ID
-- (like a Discord-style tag) — separate from the free-text username,
-- which the user can rename any time as long as it stays unique
-- (already enforced by the UNIQUE constraint on username above).
-- Adding an identity column backfills existing rows automatically.
alter table public.profiles
  add column if not exists user_number bigint generated by default as identity;

alter table public.profiles drop constraint if exists profiles_user_number_key;
alter table public.profiles add constraint profiles_user_number_key unique (user_number);

-- Defense in depth: even though the app never sends user_number in an
-- update, this trigger stops a crafted client request from changing
-- it — user_number is set once at creation and never again.
create or replace function public.lock_user_number()
returns trigger as $$
begin
...
drop trigger if exists lock_user_number_on_update on public.profiles;
create trigger lock_user_number_on_update
  before update on public.profiles
  for each row execute procedure public.lock_user_number();
...
-- Friend requests/acceptances should show up live for the other
-- person too, not just after they reload the page.
...
-- ============================================================
-- v5.4 — STATUS / STORIES (24-hour posts, like WhatsApp Status
...
create table if not exists public.statuses (
  ...
  constraint statuses_has_content check (
    (content_type = 'image' and media_url is not null)
    or (content_type = 'text' and text_content is not null)
  )
);

alter table public.statuses enable row level security;

-- You can always see your own statuses (even after they'd otherwise
-- expire, so you can still manage them); friends can see each
-- other's statuses only while they're still active.
drop policy if exists "Users can view own or friends' active statuses" on public.statuses;
create policy "Users can view own or friends' active statuses"
...
-- The status owner can see who viewed it; a viewer can see their own
-- view record (mainly so the app can check "have I already marked
-- this as viewed").
drop policy if exists "Owner or viewer can see a status view" on public.status_views;
create policy "Owner or viewer can see a status view"
